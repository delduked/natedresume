  ...
  const proxy = (
    ...
    new Proxy(extendedConsole, {
      ...
      ownKeys(target) {
        ...
        let parent2 = target;
        while (parent2 = parent2.parent) keys.push(...Reflect.ownKeys(parent2.logMethods));
        return keys;
      },
...
var AppInstance = class {
  ...
  reset() {
    this.instances = [];
    this.routeMaps = {};
    this.browserAdapter = createBrowserAdapter();
  }
  register(instance8) {
...
var createNewRouter = async (name, url2, routesMap) => {
  ...
  if (!getRoutes) return false;
  else console.log("[Routify] Pre-creating router", name, url2, routesMap);
...
var preloadUrlFromUrlPairs = async (urlPairs, routesMap) => {
  const routerPromises = urlPairs.map(async ([name, url2]) => {
    ...
    if ((currentRoute == null ? void 0 : currentRoute.url) !== url2) router.url.replace(url2);
    return router;
  });
  ...
  return routers.map((router) => {
    var _a;
    return (_a = router.activeRoute) == null ? void 0 : _a.get().load;
  });
};
...
var getMRCA = (node1, node2) => {
  ...
  const mrca = lineage1.find((node3) => lineage2.includes(node3));
    ...
    return derived(
      ...
      ($url) => (
        ...
        (pathOrNode, userParams, options) => {
          const path = typeof pathOrNode === "string" ? pathOrNode : pathOrNode == null ? void 0 : pathOrNode.path;
...
var createUrl = (fragment, router) => (
  /** @type {UrlFromString} */
  (pathOrNode, userParams = {}, options = {}) => {
    let _inputPath = typeof pathOrNode === "string" ? pathOrNode : pathOrNode == null ? void 0 : pathOrNode.path;
    ...
    const targetNode = originNode.traverse(
      offset + inputPath,
      // path
      { ...options, allowDynamic: !options.strict }
    );
    if (!targetNode) {
    ...
    const externalUrl = router.getExternalUrl(internalUrl.path + internalUrl.query).replace(/\/$/, "") || "/";
...
var isActiveUrl = (renderContext) => {
  ...
  return (pathOrNode, params2 = {}, options = {}) => {
    let _path = typeof pathOrNode === "string" ? pathOrNode : pathOrNode == null ? void 0 : pathOrNode.path;
    ...
    const wantedNode = _path.startsWith(".") ? fragment.node.traverse(_path, chainOptions) : router.rootNode.getChainTo(_path, chainOptions).pop().node;
...
var getDirection = (boundary, newRoute, oldRoute) => {
  var _a, _b;
  if (!newRoute) newRoute = get_store_value(context).route;
  ...
  if (![newNode, ...(newNode == null ? void 0 : newNode.ancestors) || []].includes(boundary)) return "last";
  if (![oldNode, ...(oldNode == null ? void 0 : oldNode.ancestors) || []].includes(boundary)) return "first";
  if (oldNode === newNode) return "same";
  if ((_a = oldNode.meta.children) == null ? void 0 : _a.includes(newNode.name)) return "higher";
  if ((_b = newNode.meta.children) == null ? void 0 : _b.includes(oldNode.name)) return "lower";
...
var LoadCache = class {
  ...
  async fetch(id2, options) {
    if (!this.map.has(id2)) this.map.set(id2, options.hydrate());
  ...
  async _handlePromise(id2, options) {
    ...
    if (typeof clear === "number") setTimeout(() => this.map.delete(id2), clear);
    else if (clear) this.map.delete(id2);
  }
};
...
var Route = class {
  ...
  constructor(router, url2, mode, state = {}) {
    ...
    /** @type {Route} */
    __publicField(this, "prevRoute");
    /** @type {Route} */
    __publicField(this, "nextRoute");
  ...
  async loadComponents() {
    ...
    const multiNodes = nodes.map(
      (node2) => node2.children.find(
        (node3) => node3.meta.isDecorator || node3.meta.isInlineWrapper
      )
    ).filter(Boolean);
  ...
  _createUrl() {
    ...
    const shouldAddSlash = this.router.options.trailingSlash === "always" || this.router.options.trailingSlash === "preserve" && this.sourceUrl.pathname.endsWith("/") || this.router.options.trailingSlash === "contextual" && [...this.allFragments].pop().node.name === "index";
    const slash = shouldAddSlash ? "/" : "";
    return populatedUrl.path.replace(/\/index$/, "") + slash + populatedUrl.query + this.sourceUrl.hash;
  }
};
...
var RNode = class {
  ...
  get ancestors() {
    let node2 = this;
    const ancestors = [];
    while (node2 = node2.parent) ancestors.push(node2);
    return ancestors;
  }
  /** @type {InstanceType['NodeConstructor']['prototype']} */
  get root() {
  ...
  traverse(path, options) {
    ...
    return isNamed ? this.root.instance.nodeIndex.find((node2) => node2.meta.name === path) : (_a = this.getChainTo(path, options)) == null ? void 0 : _a.pop().node;
  }
  /**
  ...
  getChainTo(path, options) {
    ...
    try {
      if (options.includeIndex)
        currentNodeStep.node.getDefaults().forEach((node2) => {
          const nodeStep = {
            node: node2,
            params: {},
            stepsToLeaf: [],
            // we're at the leaf, we no longer need to travel
            fragment: ""
          };
          nodeSteps.push(nodeStep);
        });
  ...
  getDefaults(childType = "children") {
    const child = this[childType].find(
      (node2) => node2.meta.isDefault || node2.name === "index"
    );
...
function instance($$self, $$props, $$invalidate) {
  ...
  if (!Component && node2.asyncModule) node2.asyncModule().then((r) => $$invalidate(1, Component = r.default));
  ...
  $$self.$$set = ($$props2) => {
    if ("node" in $$props2) $$invalidate(3, node2 = $$props2.node);
    if ("passthrough" in $$props2) $$invalidate(0, passthrough = $$props2.passthrough);
  ...
  $$self.$inject_state = ($$props2) => {
    if ("node" in $$props2) $$invalidate(3, node2 = $$props2.node);
    if ("passthrough" in $$props2) $$invalidate(0, passthrough = $$props2.passthrough);
    if ("Component" in $$props2) $$invalidate(1, Component = $$props2.Component);
  };
...
function instance2($$self, $$props, $$invalidate) {
  ...
  $$self.$$set = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
...
var Noop = class extends SvelteComponentDev {
  constructor(options) {
    super(options);
    init(this, options, instance2, create_fragment2, safe_not_equal, { context: 0, Parent: 1 });
    dispatch_dev("SvelteRegisterComponent", {
      component: this,
      tagName: "Noop",
      options,
...
var RNodeRuntime = class extends RNode {
   ...
   */
  loadModule() {
    if (!this.module && !this.asyncModule) this.module = Noop_exports;
  ...
  constructor(router) {
    ...
    const createStateEventHandler = (method) => {
      return function(data, title, url2) {
        ...
        if (routerName === false) url2 = browserAdapter.toRouter(url2, router);
    ...
    this.absorb = () => {
      var _a, _b, _c;
      const state = (_c = (_b = (_a = history.state) == null ? void 0 : _a.routify) == null ? void 0 : _b.router) == null ? void 0 : _c[router.name];
    ...
    this._popstateHandler = (event) => {
      var _a, _b;
      return router.url.pop(
        urlFromBrowser(router),
        (_b = (_a = event.state) == null ? void 0 : _a.routify) == null ? void 0 : _b.router[router.name]
      );
    };
  }
  install() {
...
var ScrollContext = class {
  ...
  async init() {
    ...
    const hash = (_a = ctx.route) == null ? void 0 : _a.sourceUrl.hash.slice(1);
    this.hashElem = hash && ((_b = globalThis.document) == null ? void 0 : _b.getElementById(hash));
    this.elem = this.hashElem || this.scrollTarget;
    this.isInstant = ((_c = ctx.route) == null ? void 0 : _c.state.dontsmoothscroll) || !ctx.wasVisible;
  ...
  async handleScrollInstructions() {
    const onDocChange = () => this.ctx.inline.scrollIntoView(this.elem, this.isInstant, {
...
var shouldScrollCb = (_scrollContext, index, ScrollContexts) => {
  return ScrollContexts.filter(
    (scrollContext) => scrollContext.scrollLock === _scrollContext.scrollLock
  ).pop() === _scrollContext;
};
var scrollToTop = (elem, scrollLock) => {
  let parent = elem.parentElement;
...
var ScrollQueue = class {
  ...
  async push(renderContext) {
    ...
    if (scrollContext.route != ((_a = this.queue[0]) == null ? void 0 : _a.scrollCtx.route)) this.queue = [];
  ...
  async processQueue() {
    ...
    const queue = this.queue.map(({ scrollCtx }) => scrollCtx).filter((scrollCtx) => scrollCtx.elem);
    ...
    const shouldScroll = (scrollCtx, index, arr) => scrollCtx.ctx.inline.shouldScroll(scrollCtx, index, arr, shouldScrollCb);
...
function instance3($$self, $$props, $$invalidate) {
  ...
  let { isRoot } = $$props;
  isRoot;
  const routeHasHashAndWeAreAtTheLeaf = (route2) => route2.sourceUrl.hash && route2.leaf === context2.fragment;
  const contextLineageIsActive = () => getLineage(context2).every((ctx) => get_store_value(ctx.isActive));
  const contextWasActive = () => {
    var _a;
    return ((_a = context2.parentContext) == null ? void 0 : _a.lastActiveChildContext) === context2 || context2.node.isRoot && context2.route.prevRoute;
  };
  $$self.$$.on_mount.push(function() {
  ...
  $$self.$$set = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
    if ("isRoot" in $$props2) $$invalidate(1, isRoot = $$props2.isRoot);
  ...
  $$self.$capture_state = () => ({
    ...
    context: context2,
    isRoot,
    routeHasHashAndWeAreAtTheLeaf,
    contextLineageIsActive,
    contextWasActive,
    route
  });
  $$self.$inject_state = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
    if ("isRoot" in $$props2) $$invalidate(1, isRoot = $$props2.isRoot);
    if ("route" in $$props2) $$invalidate(2, route = $$props2.route);
  };
  if ($$props && "$$inject" in $$props) {
    $$self.$inject_state($$props.$$inject);
  }
  $$self.$$.update = () => {
    ...
    1) {
      $: $$invalidate(2, { route } = context2, route);
    }
    if ($$self.$$.dirty & /*route, context*/
    5) {
      $: if (route && !route.state.dontScroll && (routeHasHashAndWeAreAtTheLeaf(route) || contextLineageIsActive() && !contextWasActive())) {
        const { debug } = context2.router.log;
        debug("scrolling to context", context2.node.id || "unnamed", context2);
        scrollQueue.push(context2);
      }
    }
  };
  return [context2, isRoot, route, $$scope, slots];
}
var ScrollDecorator = class extends SvelteComponentDev {
  constructor(options) {
    super(options);
    init(this, options, instance3, create_fragment3, safe_not_equal, { context: 0, isRoot: 1 });
    dispatch_dev("SvelteRegisterComponent", {
      component: this,
...
var Router = class _Router {
  ...
  async _setUrl(url2, mode, isInternal, state = {}) {
    if (!isInternal) url2 = this.getInternalUrl(url2);
    url2 = url2 || "/";
    state = state || {};
    const stackedRoute = this.history.get(state.id);
    if (stackedRoute == null ? void 0 : stackedRoute.meta.history) {
...
function create_if_block2(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      8) decoratorwrapper_changes.decorators = /*restOfDecorators*/
      ctx2[3];
      if (dirty & /*context*/
      2) decoratorwrapper_changes.context = /*context*/
      ctx2[1];
      if (dirty & /*$$scope*/
      64) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "if",
    source: "(29:4) {#if restOfDecorators.length}",
    ctx
  });
  return block;
}
function create_default_slot_1(ctx) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: "(30:8) <svelte:self decorators={restOfDecorators} {context} isRoot={false}>",
    ctx
  });
  return block;
}
function create_default_slot2(ctx) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: "(24:0) <svelte:component     this={decorator ? decorator.component : Noop}     {context}     {isRoot}     {...decorator?.props}>",
    ctx
  });
  return block;
}
function create_fragment4(ctx) {
  ...
  function switch_props(ctx2, dirty) {
    ...
    if (dirty !== void 0 && dirty & /*context, isRoot, decorator*/
    7) {
      switch_instance_props = assign(switch_instance_props, get_spread_update(switch_instance_spread_levels, [
        dirty & /*context*/
        2 && { context: (
  ...
  const block = {
    ...
    p: function update(ctx2, [dirty]) {
      ...
      ctx2[2] ? (
        ...
        const switch_instance_changes = dirty & /*context, isRoot, decorator*/
...
function instance4($$self, $$props, $$invalidate) {
  ...
  let [decorator, ...restOfDecorators] = [...decorators].sort((a, b) => (a.order || 0) - (b.order || 0));
  while (decorator && !(decorator == null ? void 0 : decorator.shouldRender({ context: context2, isRoot, decorators }))) [decorator, ...restOfDecorators] = [...restOfDecorators];
  if (isRoot) onDestroy(() => context2.onDestroy.run());
  $$self.$$.on_mount.push(function() {
    if (context2 === void 0 && !("context" in $$props || $$self.$$.bound[$$self.$$.props["context"]])) {
  ...
  $$self.$$set = ($$props2) => {
    if ("decorators" in $$props2) $$invalidate(4, decorators = $$props2.decorators);
    if ("isRoot" in $$props2) $$invalidate(0, isRoot = $$props2.isRoot);
    if ("context" in $$props2) $$invalidate(1, context2 = $$props2.context);
  ...
  $$self.$inject_state = ($$props2) => {
    ...
    if ("isRoot" in $$props2) $$invalidate(0, isRoot = $$props2.isRoot);
    if ("context" in $$props2) $$invalidate(1, context2 = $$props2.context);
    if ("decorator" in $$props2) $$invalidate(2, decorator = $$props2.decorator);
    if ("restOfDecorators" in $$props2) $$invalidate(3, restOfDecorators = $$props2.restOfDecorators);
  };
  if ($$props && "$$inject" in $$props) {
    $$self.$inject_state($$props.$$inject);
  }
...
var DecoratorWrapper = class extends SvelteComponentDev {
  constructor(options) {
    super(options);
    init(this, options, instance4, create_fragment4, safe_not_equal, { decorators: 4, isRoot: 0, context: 1 });
    dispatch_dev("SvelteRegisterComponent", {
      component: this,
...
var defaultScrollIntoView = (elem, isInstant, options) => {
  ...
  if (options.boundary) {
    return import("./dist-U5NGSLUW.js").then((r) => {
...
var wrapperDefaults = {
  recursive: false,
  shouldRender: ({ context: context2 }) => context2.isInline
};
...
var fetchIndexNode = (node2) => node2.navigableChildren.find((node3) => node3.meta.isDefault);
var addFolderDecorator = (decorators, context2) => {
  var _a;
  const folderDecorator = (_a = context2["node"]) == null ? void 0 : _a.children.find((node2) => node2.meta.isDecorator);
  const push = () => {
    ...
    decorators.push({
      ...
      shouldRender: options.shouldRender ?? folderDecorator.meta.shouldRender ?? (() => true),
      props: options.props || {}
    });
  };
  if (folderDecorator) {
...
var addFolderWrapper = (decorators, context2) => {
  var _a;
  const inlineWrapper = (_a = context2["node"]) == null ? void 0 : _a.children.find(
    (node2) => node2.meta.isInlineWrapper
  );
...
var defaultScrollLock = (renderContext, scrollTarget, scrollContext) => {
  var _a, _b;
  return [
    !renderContext.isInline && ((_b = get_store_value((_a = findNearestInlineContext(renderContext)) == null ? void 0 : _a.elem)) == null ? void 0 : _b.parent),
...
var RouterContext = class {
  ...
  constructor({ router }) {
    ...
    __publicField(this, "anchorLocation");
    this.router = router;
    this.route = router.activeRoute.get();
    this.anchorLocation = router.anchor;
  }
  get descendants() {
    return get_store_value(this.childContexts).reduce(
      (acc, context2) => [...acc, ...context2.descendants],
      get_store_value(this.childContexts)
    );
  }
  /**
  ...
  buildChildContexts(options, newDecorators) {
    ...
    const children = [...(node2 == null ? void 0 : node2.navigableChildren) || []];
    ...
    Object.entries(paramsPool).forEach(([key, values]) => {
      const sourceIndex = children.findIndex((node3) => node3.paramKeys.includes(key));
...
var RenderContext = class _RenderContext extends RouterContext {
  ...
  get allProps() {
    var _a;
    return Object.assign({}, (_a = this.parentContext) == null ? void 0 : _a.allProps, this.props);
  }
  setToActive() {
  ...
  update(activeSiblingContext) {
    ...
    if (!get_store_value(this.isVisible)) this.elem.set(null);
    this.updateParams();
  }
...
function create_if_block_1(ctx) {
  ...
  const block = {
    c: function create() {
      ...
      add_location(div, file, 86, 4, 2988);
    },
    m: function mount(target, anchor) {
...
function create_if_block3(ctx) {
  ...
  const block = {
    c: function create() {
      ...
      if (default_slot) default_slot.c();
      set_attributes(div, div_data);
      add_location(div, file, 82, 4, 2848);
    },
    m: function mount(target, anchor) {
      insert_dev(target, div, anchor);
      if (default_slot) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "if",
    source: "(82:0) {#if location === 'wrapper'}",
    ctx
  });
  return block;
}
function create_if_block_3(ctx) {
  ...
  const block = {
    c: function create() {
      div = element("div");
      set_attributes(div, div_data);
      add_location(div, file, 90, 8, 3112);
    },
    m: function mount(target, anchor) {
      insert_dev(target, div, anchor);
      ctx[11](div);
...
function create_if_block_2(ctx) {
  ...
  const block = {
    c: function create() {
      ...
      add_location(div, file, 98, 19, 3288);
    },
    m: function mount(target, anchor) {
      insert_dev(target, div, anchor);
    },
...
function instance5($$self, $$props, $$invalidate) {
  const omit_props_names = ["context", "onMount"];
  let $$restProps = compute_rest_props($$props, omit_props_names);
  let { $$slots: slots = {}, $$scope } = $$props;
  validate_slots("AnchorDecorator", slots, ["default"]);
  let { context: context2 } = $$props;
  const location2 = context2.anchorLocation;
  let { onMount: onMount2 = (x) => x } = $$props;
  let elem;
  let mounted = false;
  const name = context2 instanceof RenderContext ? context2.node.name || "unnamed" : context2.router.name || "defaultRouter";
  ...
  const handlers = {
    wrapper: (elem2) => {
      elem2.dataset.routifyAnchorWrapper = name;
      onMount2(elem2);
    },
    parent: (elem2) => {
      elem2.dataset.routifyAnchorParent = name;
      onMount2(elem2.parentElement);
    },
    header: (elem2) => {
      elem2.dataset.routifyAnchorHeader = name;
      onMount2(elem2.parentElement, elem2);
    },
    firstChild: (elem2) => {
      const nextSib = nextValidSibling(elem2);
      nextSib.dataset.routifyAnchorNextSibling = name;
      onMount2(elem2.parentElement, nextSib);
    },
    custom: (elem2) => {
      const res = location2(elem2);
      ...
      if (res.dataset.hasOwnProperty("routifyAnchorLocator")) {
        ...
        (context2["node"] || context2.router).log.warn(warn);
      }
      onMount2(elem2.parentElement, res);
    }
  };
  onMount(() => {
    if (mounted) return;
    const handlerName = typeof location2 === "function" ? "custom" : location2;
    if (!handlers[handlerName]) throw new Error(`Anchor location "${handlerName}" is not valid. Wrapper options: 'wrapper', 'parent', 'header', 'firstChild', or a function that returns a valid element.`);
  ...
  $$self.$$set = ($$new_props) => {
    ...
    if ("context" in $$new_props) $$invalidate(5, context2 = $$new_props.context);
    if ("onMount" in $$new_props) $$invalidate(6, onMount2 = $$new_props.onMount);
  ...
  $$self.$inject_state = ($$new_props) => {
    if ("context" in $$props) $$invalidate(5, context2 = $$new_props.context);
    if ("onMount" in $$props) $$invalidate(6, onMount2 = $$new_props.onMount);
    if ("elem" in $$props) $$invalidate(0, elem = $$new_props.elem);
...
var AnchorDecorator = class extends SvelteComponentDev {
  constructor(options) {
    super(options);
    init(this, options, instance5, create_fragment5, safe_not_equal, { context: 5, onMount: 6 });
    dispatch_dev("SvelteRegisterComponent", {
      component: this,
...
function create_catch_block(ctx) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "catch",
    source: "(1:0) <script>     import Compose from './ComposeFragments.svelte'     import DecoratorWrapper from './DecoratorWrapper.svelte'     import AnchorDecorator from '../decorators/AnchorDecorator.svelte'     import { setRoutifyFragmentContext, waitFor }",
    ctx
  });
  return block;
}
function create_then_block(ctx) {
  let previous_key = (
    /*fragment*/
    ctx[1].node.meta["param-is-page"] && JSON.stringify(
      /*fragment*/
      ctx[1].params
    )
  );
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      if (dirty & /*fragment*/
      2 && safe_not_equal(previous_key, previous_key = /*fragment*/
      ctx2[1].node.meta["param-is-page"] && JSON.stringify(
        /*fragment*/
        ctx2[1].params
      ))) {
        group_outros();
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "then",
    source: "(52:43)          {#key fragment.node.meta['param-is-page'] && JSON.stringify(fragment.params)}",
    ctx
  });
  return block;
}
function create_if_block_12(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      if (dirty & /*inline, multi, decorator, props, options, scrollLock, anchor, context*/
      130049) compose_changes.options = {
        inline: (
          /*inline*/
          ctx2[11] || /*multi*/
          ctx2[12]
      ...
      if (dirty & /*context*/
      1) compose_changes.context = /*context*/
      ctx2[0];
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "if",
    source: "(69:21) {#if $childFragments.length || ((hasInlineChildren || inline || multi) && !(inline || multi)?.single)}",
    ctx
  });
  return block;
}
function create_default_slot_2(ctx) {
  ...
  let if_block = (
    ...
    ctx[12]) && !/*inline*/
    ((_a = ctx[11] || /*multi*/
    ctx[12]) == null ? void 0 : _a.single)) && create_if_block_12(ctx)
  );
  const block = {
    c: function create() {
    ...
    p: function update(ctx2, dirty) {
      ...
      if (
        ...
        ctx2[12]) && !/*inline*/
        ((_a2 = ctx2[11] || /*multi*/
        ctx2[12]) == null ? void 0 : _a2.single)
      ) {
        if (if_block) {
          if_block.p(ctx2, dirty);
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: "(57:16) <svelte:component                     this={context.node.module.default}                     {...compProps}                     {...context.props}                     {context}                     let:props                     let:inline                     let:multi                     let:decorator                     let:anchor                     let:options                     let:scrollLock                     >",
    ctx
  });
  return block;
}
function create_if_block4(ctx) {
  ...
  const block = {
    c: function create() {
      div = element("div");
      add_location(div, file2, 82, 20, 3645);
    },
    m: function mount(target, anchor) {
      insert_dev(target, div, anchor);
      if (!mounted) {
...
function create_default_slot_12(ctx) {
    ...
    let switch_instance_props = {
      $$slots: {
        default: [
          ...
          ({ props, inline, multi, decorator, anchor, options, scrollLock }) => ({
            10: props,
            11: inline,
            12: multi,
            13: decorator,
          ...
          ({ props, inline, multi, decorator, anchor, options, scrollLock }) => (props ? 1024 : 0) | (inline ? 2048 : 0) | (multi ? 4096 : 0) | (decorator ? 8192 : 0) | (anchor ? 16384 : 0) | (options ? 32768 : 0) | (scrollLock ? 65536 : 0)
        ]
      },
      $$scope: { ctx: ctx2 }
    };
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      if (dirty & /*context*/
      1 && switch_value !== (switch_value = /*context*/
      ctx2[0].node.module.default)) {
        if (switch_instance) {
          group_outros();
          const old_component = switch_instance;
          transition_out(old_component.$$.fragment, 1, 0, () => {
        ...
        9 ? get_spread_update(switch_instance_spread_levels, [
          dirty & /*compProps*/
          8 && get_spread_object(
            /*compProps*/
            ctx2[3]
        ...
        if (dirty & /*$$scope, inline, multi, decorator, props, options, scrollLock, anchor, context, $childFragments, hasInlineChildren*/
        261201) {
          switch_instance_changes.$$scope = { dirty, ctx: ctx2 };
        }
        switch_instance.$set(switch_instance_changes);
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: "(55:12) <AnchorDecorator {context} onMount={initialize}>",
    ctx
  });
  return block;
}
function create_key_block(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      if (dirty & /*context*/
      1) anchordecorator_changes.context = /*context*/
      ctx2[0];
      if (dirty & /*$$scope, isMounted, context, compProps, $childFragments, hasInlineChildren*/
      131165) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "key",
    source: "(53:8) {#key fragment.node.meta['param-is-page'] && JSON.stringify(fragment.params)}",
    ctx
  });
  return block;
}
...
function create_default_slot3(ctx) {
  ...
  handle_promise(promise = /*context*/
  ctx[0].node.loadModule(), info);
  const block = {
    c: function create() {
    ...
    p: function update(new_ctx, dirty) {
      ...
      info.ctx = ctx;
      if (dirty & /*context*/
      1 && promise !== (promise = /*context*/
      ctx[0].node.loadModule()) && handle_promise(promise, info)) {
      } else {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: "(51:0) <svelte:component this={context.decorators.length ? DecoratorWrapper : Noop} {context}>",
    ctx
  });
  return block;
}
function create_fragment6(ctx) {
  ...
  var switch_value = (
    /*context*/
    ctx[0].decorators.length ? DecoratorWrapper_default : Noop_default
  );
  function switch_props(ctx2, dirty) {
    return {
      props: {
  ...
  const block = {
    ...
    p: function update(ctx2, [dirty]) {
      if (dirty & /*context*/
      1 && switch_value !== (switch_value = /*context*/
      ctx2[0].decorators.length ? DecoratorWrapper_default : Noop_default)) {
        if (switch_instance) {
          group_outros();
          const old_component = switch_instance;
          transition_out(old_component.$$.fragment, 1, 0, () => {
        ...
        if (dirty & /*context*/
        1) switch_instance_changes.context = /*context*/
        ctx2[0];
        if (dirty & /*$$scope, context, fragment, isMounted, compProps, $childFragments, hasInlineChildren*/
        131167) {
          switch_instance_changes.$$scope = { dirty, ctx: ctx2 };
        }
        switch_instance.$set(switch_instance_changes);
...
function instance6($$self, $$props, $$invalidate) {
  ...
  let compProps;
  let $childFragments, $$unsubscribe_childFragments = noop, $$subscribe_childFragments = () => ($$unsubscribe_childFragments(), $$unsubscribe_childFragments = subscribe(childFragments, ($$value) => $$invalidate(6, $childFragments = $$value)), childFragments);
  $$self.$$.on_destroy.push(() => $$unsubscribe_childFragments());
  let { $$slots: slots = {}, $$scope } = $$props;
  ...
  const updateRenderContext = (elem, newMeta) => {
    var _a;
    if (elem) elem["__routify_meta"] = {
      ...elem && elem["__routify_meta"],
      renderContext: {
        ...(_a = elem["__routify_meta"]) == null ? void 0 : _a.renderContext,
        ...newMeta
      }
    };
    return elem;
  ...
  const initialize = async (parent, anchor) => {
    ...
    if (anchor) anchor = updateRenderContext(anchor, { anchor: context2 });
  };
  const childMounted = (_elem) => {
    $$invalidate(2, isMounted = true);
  ...
  $$self.$inject_state = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
    if ("isMounted" in $$props2) $$invalidate(2, isMounted = $$props2.isMounted);
    if ("fragment" in $$props2) $$invalidate(1, fragment = $$props2.fragment);
    if ("compProps" in $$props2) $$invalidate(3, compProps = $$props2.compProps);
    if ("hasInlineChildren" in $$props2) $$invalidate(4, hasInlineChildren = $$props2.hasInlineChildren);
    if ("childFragments" in $$props2) $$subscribe_childFragments($$invalidate(5, childFragments = $$props2.childFragments));
  };
  if ($$props && "$$inject" in $$props) {
    $$self.$inject_state($$props.$$inject);
  }
  $$self.$$.update = () => {
    ...
    if ($$self.$$.dirty & /*context*/
    1) {
      $: $$subscribe_childFragments($$invalidate(
        5,
        { childFragments, fragment } = context2,
        // grab the stores
        childFragments,
        ($$invalidate(1, fragment), $$invalidate(0, context2))
      ));
    }
    if ($$self.$$.dirty & /*context*/
    1) {
    ...
    if ($$self.$$.dirty & /*fragment, context*/
    3) {
      $: $$invalidate(3, compProps = {
        ...fragment.params,
        ...(_a = fragment.load) == null ? void 0 : _a.props,
        ...context2.props
      });
    }
  };
...
function create_each_block(key_1, ctx) {
  ...
  const block = {
    ...
    p: function update(new_ctx, dirty) {
      ...
      if (dirty & /*$childContexts, decoratorsReady, wrappersReady*/
      14) renderfragment_changes.context = /*context*/
      ctx[0];
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "each",
    source: "(75:0) {#each $childContexts.filter(cc => decoratorsReady && wrappersReady && get(cc.isVisible)) as context (context)}",
    ctx
  });
  return block;
}
...
function instance7($$self, $$props, $$invalidate) {
  ...
  const inlineWrapper = ((_a = options.inline) == null ? void 0 : _a["wrapper"]) && normalizeWrapper(options.inline["wrapper"]);
  const recursiveDecorators = context2.decorators.filter((deco) => deco.recursive);
  const newDecorators = pushToOrReplace(recursiveDecorators, [decorator, inlineWrapper]).filter(Boolean).map(normalizeDecorator);
  let decoratorsReady = !((_b = addFolderDecorator(newDecorators, context2)) == null ? void 0 : _b.then(() => {
    $$invalidate(1, decoratorsReady = true);
  }));
  let wrappersReady = !((_c = addFolderWrapper(newDecorators, context2)) == null ? void 0 : _c.then(() => {
    $$invalidate(2, wrappersReady = true);
  }));
  context2.buildChildContexts(options, newDecorators);
  const { childContexts } = context2;
  validate_store(childContexts, "childContexts");
  component_subscribe($$self, childContexts, (value) => $$invalidate(3, $childContexts = value));
  const _handleChildren = (childFragments2) => {
    ...
    if (childFragments2.length && context2.route) setActiveChildContext(context2);
    else
  ...
  const setActiveChildContext = (context3, rebuild) => {
    ...
    if (!toBeActiveChildContext) {
      if (rebuild) handleRebuildError(context3, childContexts2);
  ...
  const func = (cc) => decoratorsReady && wrappersReady && get_store_value(cc.isVisible);
  $$self.$$set = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
    if ("options" in $$props2) $$invalidate(6, options = $$props2.options);
  };
  $$self.$capture_state = () => ({
    get: get_store_value,
    pushToOrReplace,
  ...
  $$self.$inject_state = ($$props2) => {
    if ("context" in $$props2) $$invalidate(0, context2 = $$props2.context);
    if ("options" in $$props2) $$invalidate(6, options = $$props2.options);
    if ("decoratorsReady" in $$props2) $$invalidate(1, decoratorsReady = $$props2.decoratorsReady);
    if ("wrappersReady" in $$props2) $$invalidate(2, wrappersReady = $$props2.wrappersReady);
  };
  if ($$props && "$$inject" in $$props) {
    $$self.$inject_state($$props.$$inject);
  }
...
function create_if_block5(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, dirty) {
      ...
      if (dirty & /*context*/
      2) component_changes.context = /*context*/
      ctx2[1];
      if (dirty & /*decorator*/
      1) component_changes.options = { decorator: (
        /*decorator*/
        ctx2[0]
      ) };
...
function create_default_slot4(ctx) {
  ...
  dispatch_dev("SvelteRegisterBlock", {
    ...
    type: "slot",
    source: '(143:0) <AnchorDecorator onMount={initialize} style=\\"display: contents\\" {context}>',
    ctx
  });
  return block;
}
function create_fragment8(ctx) {
  ...
  const block = {
    ...
    p: function update(ctx2, [dirty]) {
      const anchordecorator_changes = {};
      if (dirty & /*context*/
      2) anchordecorator_changes.context = /*context*/
      ctx2[1];
...
function instance_1($$self, $$props, $$invalidate) {
  ...
  const initialize = (elem) => {
    var _a;
    elem = router.anchor === "parent" || router.anchor === "wrapper" ? elem : elem.parentElement;
  ...
  const handleHover = (event) => {
    var _a, _b, _c;
    const { url: url3, state } = getUrlFromEvent(event);
  ...
  const handleClick = (event) => {
    ...
    if (typeof url3 === "string") router.url.push(url3, state);
  };
  ...
  $$self.$$set = ($$props2) => {
    if ("router" in $$props2) $$invalidate(5, router = $$props2.router);
    if ("routes" in $$props2) $$invalidate(6, routes = $$props2.routes);
    if ("decorator" in $$props2) $$invalidate(0, decorator = $$props2.decorator);
    if ("urlReflector" in $$props2) $$invalidate(7, urlReflector = $$props2.urlReflector);
    if ("instance" in $$props2) $$invalidate(8, instance8 = $$props2.instance);
    if ("urlRewrite" in $$props2) $$invalidate(9, urlRewrite = $$props2.urlRewrite);
    if ("url" in $$props2) $$invalidate(10, url2 = $$props2.url);
    if ("name" in $$props2) $$invalidate(11, name = $$props2.name);
    if ("rootNode" in $$props2) $$invalidate(12, rootNode = $$props2.rootNode);
    if ("passthrough" in $$props2) $$invalidate(13, passthrough = $$props2.passthrough);
    if ("beforeRouterInit" in $$props2) $$invalidate(14, beforeRouterInit = $$props2.beforeRouterInit);
    if ("afterRouterInit" in $$props2) $$invalidate(15, afterRouterInit = $$props2.afterRouterInit);
    if ("beforeUrlChange" in $$props2) $$invalidate(16, beforeUrlChange2 = $$props2.beforeUrlChange);
    if ("afterUrlChange" in $$props2) $$invalidate(17, afterUrlChange2 = $$props2.afterUrlChange);
    if ("transformFragments" in $$props2) $$invalidate(18, transformFragments = $$props2.transformFragments);
    if ("onDestroy" in $$props2) $$invalidate(19, onDestroy2 = $$props2.onDestroy);
    if ("plugins" in $$props2) $$invalidate(20, plugins = $$props2.plugins);
    if ("queryHandler" in $$props2) $$invalidate(21, queryHandler = $$props2.queryHandler);
    if ("anchor" in $$props2) $$invalidate(22, anchor = $$props2.anchor);
    if ("clickHandler" in $$props2) $$invalidate(23, clickHandler = $$props2.clickHandler);
  };
  $$self.$capture_state = () => ({
    Router,
  ...
  $$self.$inject_state = ($$props2) => {
    ...
    if ("anchor" in $$props2) $$invalidate(22, anchor = $$props2.anchor);
    if ("clickHandler" in $$props2) $$invalidate(23, clickHandler = $$props2.clickHandler);
    if ("activeRoute" in $$props2) $$subscribe_activeRoute($$invalidate(3, activeRoute2 = $$props2.activeRoute));
  };
  if ($$props && "$$inject" in $$props) {
    $$self.$inject_state($$props.$$inject);
  }
  $$self.$$.update = () => {
    ...
    1056) {
      $: if (url2 && url2 !== router.url.internal()) router.url.replace(url2);
    }
    ...
    4) {
      $: $$invalidate(1, context2.route = $activeRoute, context2);
    }
